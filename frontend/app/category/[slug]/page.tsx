    ...
    const fetchResults = async () => {
      try {
        ...
        const {
          ...
          .from("ads")
          .select(
            "id, title, description, status, created_at, category:categories!inner(name, slug), city:cities(name), package:packages(price), seller:users(full_name, email, is_verified_seller), media:ad_media(original_url, thumbnail_url)",
            { count: "exact" },
          )
          .eq("status", "published")
        ...
        const rows = (data || []) as unknown as CategorySearchRow[];
